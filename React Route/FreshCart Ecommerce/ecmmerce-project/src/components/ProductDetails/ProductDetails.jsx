    ...
    <div className='row my-5'>
      <div className='col-12 col-sm-12 col-md-6'>
        <div className='productSlider'>
          {
            (ProductDetail.images) ? <ProductImagesSlider propImages={ProductDetail.images} /> : null
          ...
          <div className='productMianInfo'>
              ...
              ProductDetail ?
                <>
                  <div className='d-flex justify-content-between align-items-center my-3'>
                    ...
                    <h3 className='display-6 text-body-secondary'><b>{ProductDetail.price}$</b></h3>
                  </div>
            ...
            {ProductDetail ?
              <div className='text-start'>
                <mark>{ProductDetail.ratingsAverage} <i className="fa-solid fa-star"></i></mark>
              ...
              </div> : ''}
            {
              <>
                <div className='d-flex align-items-center gap-2 my-3'>
                  <i
                    className={`fa-solid fa-circle-dot ${ProductDetail.quantity > 10 ? 'text-success' : 'text-danger'}`}
                  ></i>
                  <span>
                    Quantity: {ProductDetail.quantity}
                    {ProductDetail.quantity > 100 && ' (In Stock)'}
                    {ProductDetail.quantity <= 100 && ProductDetail.quantity > 0 && ' (Low Stock)'}
                    {ProductDetail.quantity === 0 && ' (Out of Stock)'}
                  </span>
                </div>
                <div className="alert alert-primary my-2" style={{ maxWidth: '300px' }} role="alert">
                  {ProductDetail.sold >= 1000 ? `Sold +${ProductDetail.sold} times` : `Sold ${ProductDetail.sold} times`}
                </div>
              </>
            }
            <form>
                ...
                <button type="submit" className="btn btn-primary w-100 btn-lg">Add to cart</button>
            </form>
          </div>
        </div>
      </div>
